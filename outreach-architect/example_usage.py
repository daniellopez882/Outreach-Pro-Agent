...
async def example_full_workflow():
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
            ...
            if result["status"] == "completed":
                ...
                for idx, res in enumerate(results[:2], 1):  # Show first 2
                    ...
                    if "stages" in res and "email_generation" in res["stages"]:
                        ...
                        print(
                            f"     Personalization: {len(email.get('personalization_elements', []))} elements"
                        )
                        print(
                            f"     Quality Score: {res['stages']['quality_check'].get('quality_score', 0):.2f}"
                        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"  Response Rate: {stats['response_rate']}% (Target: {stats['target_response_rate']})"
            )
