...
class OutreachOrchestrator:
    ...
    async def process_lead(
        self, lead: Lead, company_context: str, value_proposition: str, auto_send: bool = False
    ) -> dict[str, Any]:
        """
        ...
        result = {"lead_id": lead.id, "lead_name": lead.name, "status": "processing", "stages": {}}
        ...
        try:
            ...
            for attempt in range(max_attempts):
                ...
                email = await self._generate_email(
                    lead, analysis, company_context, value_proposition
                )
                ...
                logger.warning(
                    f"Quality score {quality_check['quality_score']} below threshold. issues: {quality_check['issues']}"
                )
    ...
    async def _enrich_lead_data(self, lead: Lead) -> dict[str, Any]:
        """
        Gather what is known about the lead, through the configured providers.

        This used to call a LinkedIn scraper that logged in with a username and
        password. That path was non-functional (no CSRF token, redirects not
        followed, and a success check that raised TypeError into a bare except)
        and breached the LinkedIn User Agreement. Enrichment now goes through
        the provider interface in ``enrichment/``, whose shipped providers are
        compliant by construction. See docs/data-sourcing.md.
        """
        ...
        for provider_name in settings.enrichment_provider_list:
            try:
                provider = get_provider(provider_name)
            except Exception as exc:
                errors.append(f"{provider_name}: unavailable ({exc})")
                continue

            try:
                person = await provider.enrich_person(lead.email or lead.name, lead=lead_fields)
                if person.ok:
                    results.append(person)
                elif person.errors:
                    errors.extend(f"{provider_name}: {e}" for e in person.errors)

                if lead.company:
                    company = await provider.enrich_company(
                        lead.company,
                        lead=lead_fields,
                        company_website=lead_fields.get("company_website"),
                    )
                    if company.ok:
                        results.append(company)
                    elif company.errors:
                        errors.extend(f"{provider_name}: {e}" for e in company.errors)
            except Exception as exc:
                logger.error(f"Provider {provider_name} raised: {exc}")
        ...
        # Company intelligence is a separate, non-scraping source.
        if lead.company:
            try:
                company_data = await company_intel.enrich_company_data(
                    lead.company, lead_fields.get("company_website")
                )
                if company_data:
                    results.append(
                        EnrichmentResult(
                ...
                logger.error(f"Company intelligence failed: {exc}")
        ...
        try:
            ...
            logger.error(f"Could not persist enrichment timestamp: {exc}")
            self.db.rollback()

        # Partial enrichment is still useful, so a provider failing does not
        # fail the whole step. The previous version returned status='failed'
        # with an empty payload if any single call raised.
        return {
            "status": "success" if results else ("partial" if errors else "empty"),
            "data": {r.source: r.data for r in results},
    ...
    async def _generate_email(
        self, lead: Lead, analysis: dict, company_context: str, value_proposition: str
    ) -> dict[str, Any]:
        """Generate personalized email using Kimi"""

        lead_data = {"name": lead.name, "company": lead.company, "job_title": lead.job_title}
    ...
    async def _create_campaign(
        self, lead: Lead, email: dict, analysis: dict, quality_check: dict
    ) -> OutreachCampaign:
    ...
    async def batch_process_leads(
        ...
        async def process_with_semaphore(lead):
            async with semaphore:
                return await self.process_lead(lead, company_context, value_proposition, auto_send)

        # Process all leads concurrently (with limit)
        results = await asyncio.gather(
            *[process_with_semaphore(lead) for lead in leads], return_exceptions=True
        )
        ...
        stats = {
            "total": len(results),
            "successful": sum(
                1 for r in results if r.get("status") == "ready" or r.get("status") == "sent"
            ),
